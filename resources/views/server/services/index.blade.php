...
<div class="container" {{ $rtl }}>
	<div class="row">
		...
		<div class="col">
			<button class="btn btn-light border mb-2 mt-5 pt-0 pb-0 float-right" onclick="window.location.assign('{{ url('server/service/create') }}')">{{ __('server.newService') }}</button>
				      ...
				      <td>
				      	<form method="POST" action="{{ url('server/service/'.$service->id) }}">
							...
							{{ method_field('DELETE') }}
					      	...
					      	<a class="btn btn-success" onclick="window.location.assign('{{ url('server/service/'.$service->id.'') }}')">{{ __('server.view') }}</a>
					      	<a class="btn btn-primary" onclick="window.location.assign('{{ url('server/service/'.$service->id.'/edit') }}')">{{ __('server.edit') }}</a>
					      	<button type="submit" class="btn btn-danger">{{ __('server.delete') }}</button>
				        </form>
				      </td>
				    </tr>	
