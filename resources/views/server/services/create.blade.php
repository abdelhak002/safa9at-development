...
<div class="container">
		...
		<form action="{{ url('server/service') }}" method="POST" enctype="multipart/form-data">
			...
			<div class="form-row">
				...
				<div class="form-group col">
						<label for="sub_category">{{ __('server.subCategories') }}</label>
						<select class="form-control" id="sub_category" name="sub_category"  required>
								<option>{{ __('server.selectSubCategory') }}</option>
			...
			<div class="form-row">
				<div class="form-group col">
					<label for="price">{{__('server.price')}}</label>
					<input type="number" class="form-control" id="price" name="price" value=" {{ old('price') }}">
				</div>
				<div class="form-group col">
					<label for="duration">{{ __('server.duration') }}</label>
				...
				<div class="form-group col">
					<label for="remaining">{{ __('server.quantity') }}</label>
			...
			<div class="form-row">
				<div class="form-group col">
					<label for="p_method">{{ __('deposit.paymentMethod') }} (safa9at)</label>
			...
			<div class="form-group">
				<label for="desc">{{ __('server.description') }}</label>
			...
			<button class="btn btn-dark float-right">{{ __('server.create') }}</button>
		</form>
</div>
